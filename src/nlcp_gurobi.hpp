...
////////////////////////////////////////////////////////////////////////
//! mc::NLCP_GUROBI is a C++ class for constraint projection using
...
( T*P )
{
  ...
  switch( NLGO_GUROBI<T>::get_status() ){
   ...
   case GRB_OPTIMAL:
#endif
    //std::cout << "Feasibility:" << NLGO_GUROBI<T>::get_objective() << std::endl;
    if( NLGO_GUROBI<T>::get_objective() > options.LPOPTIMTOL )
  ...
  const bool reset, const bool feastest )
{
  ...
  for( nred=0; nred < options.DOMREDMAX; nred++ ){
    ...
    else        NLGO_GUROBI<T>::_update_polrelax( P, tvar, feastest );
    ...
    P0.assign( P, P+_nvar );
    NLGO_GUROBI<T>::_contract( P, tvar, inc, false, feastest );
    if( !NLGO_GUROBI<T>::optimal_status() ) return status;
