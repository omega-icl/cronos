...
class ODEBND_BASE: public virtual BASE_DE
{
 ...
 protected:
  ...
  //! @brief Static function converting interval bounds to GSL array
  template <typename REALTYPE> static void _I2vec
    ( const unsigned nx, const T*Ix, REALTYPE*vec );
  ...
  template <typename REALTYPE> static void _vec2I
    ( const REALTYPE*vec, const unsigned nx, T*Ix );
  ...
  template <typename VRES> static void _record
    ( std::ofstream&bndsta, const VRES&ressta, const unsigned iprec=5 );
  ...
  _nVAR(0), _pVAR(0)
{
  ...
  _Ir = _Idfdx = _Irdot = 0;
  _xLdot = _xUdot = _RxLdot = _RxUdot = _Rqdot = 0;
...
( const OPT&options, const unsigned np, const PVT* PMp, const unsigned ns )
{
  ...
  delete[] _Rqdot;  _Rqdot  = _nq? new double[_nq]: 0;
...
( std::ofstream&bndsta, const VRES&ressta, const unsigned iprec )
{
  if( !bndsta ) return;
  ...
  auto it = ressta.begin();
  for( ; it != ressta.end(); ++it ){
