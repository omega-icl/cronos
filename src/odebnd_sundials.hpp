...
\page page_ODEBND Bounding the Reachable Sets of Parametric ODEs using SUNDIALS and MC++ (Continuous-Time Set-Propagation)
...
*/
      ...
      if( Ixk ){
        if( !Ixk[_istg+1] ) Ixk[_istg+1] = new T[_nx+_nq];
        for( unsigned ix=0; ix<_nx+_nq; ix++ )
      ...
      if( _nf
       ...
       && !_FCT_I_STA( options, _pos_fct, _t, NV_DATA_S( _Nf ) ) )
        { _END_STA(); return FATAL; }
    }
  ...
  _cv_flag = CVodeGetNumSteps( _cv_mem, &nstp );
  stats_sta.numSteps += nstp;
#ifdef MC__ODEBND_SUNDIALS_DEBUG
...
( const PVT*PMp, PVT**PMxk, E*ERxk, PVT*PMf, const bool store, std::ostream&os )
{
  ...
  try{
    ...
    if( options.DISPLAY >= 1 ){
      ...
      //  _print_interm( _t, _nx, _PMx, _Er, "x", os );
      //else
...
*/
     ...
     _GET_PM_STA( options, NV_DATA_S(_Nx), _nq && _Nq? NV_DATA_S(_Nq): 0 );
      ...
      // Display / return stage results
      if( PMxk ){
        if( !PMxk[_istg+1] ) PMxk[_istg+1] = new PVT[_nx+_nq];
        for( unsigned ix=0; ix<_nx+_nq; ix++ )
