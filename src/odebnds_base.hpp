...
protected:
  ...
  //using ODEBND_BASE<T,PMT,PVT>::_pX;
  //using ODEBND_BASE<T,PMT,PVT>::_pY;
  //using ODEBND_BASE<T,PMT,PVT>::_pP;
  //using ODEBND_BASE<T,PMT,PVT>::_pQ;
  //using ODEBND_BASE<T,PMT,PVT>::_pT;
  ...
  _pADJCC(0), _pADJDTC(0)
{
  ...
  _MVZd  = _MVZy  = _MVZVAR  = _MVZz  = _MVZp = 0;
...
{ 
  ...
  for( unsigned ip=0; ip<_np; ip++ ) _pVAR[_nx + _nx + ip] = _pP[ip];
  ...
  _Iq = _Ip + _np;
  _Iyq = _Iq + _nq;
  for( unsigned ip=0; ip<_np; ip++ ) _Ip[ip] = Ip[ip];
  ...
  delete[] _Iyqdot; _Iyqdot = new T[_np];
...
( const OPT&options, const double t, REALTYPE*vec, unsigned pos_fct, unsigned ifct )
{
  ...
  case OPT::NONE:
  case OPT::DINEQ:
    //for(int i=0; i<_nz; i++){std::cout << "IVAR["<<i+_nx<<"] = " << _IVAR[i+_nx] << std::endl;}
    _pDAG->eval( _opADJTC, _nx, _pADJTC, _Iy, _nz+1, _pVAR+_nx, _IVAR+_nx );
    //???_pDAG->eval( _opADJTC, _PMADJTC, _nx, _pADJTC, _Iy, _nz+1, _pVAR+_nx, _IVAR+_nx );
    _I2vec( _nx, _Iy, vec );
    //for(int i=0; i<_nx; i++){std::cout << "Iy["<<i<<"] = " << _Iy[i] << std::endl;}
    //??_I2vec( _nx, _Iy, _np, _Iyq, _vec_adj+_pos_adj );
    break;
...
( const OPT&options, REALTYPE*vec )
{
  ...
  _pDAG->eval( _np, _pADJTC+_nx, _Iyq, _nz+1, _pVAR+_nx, _IVAR+_nx );
  _I2vec( _np, _Iyq, vec );
  return true;
}
...
()
{
  ...
  delete[] _pADJTC; _pADJTC = _pDAG->FAD( 1, &pHAM, _np, _pVAR+_nx+_nx );
#else
  delete[] _pADJTC; _pADJTC = _pDAG->BAD( 1, &pHAM, _np, _pVAR+_nx+_nx );
#endif

  // Add initial state contribution to derivative bounds
  _pDAG->eval( _np, _pADJTC, _Iyq, _nVAR-_np, _pVAR, _IVAR, true );
...
( const OPT&options, const double t, REALTYPE*vec, unsigned pos_fct, unsigned ifct )
{
  ...
  case OPT::NONE:
  case OPT::DINEQ:
    _pDAG->eval( _nx, _pADJCC, _Ixdot, _nVAR-_np, _pVAR, _IVAR );
    _pDAG->eval( _np, _pADJTC+_nx, _Iyq, _nVAR-_np, _pVAR, _IVAR, true );
    _I2vec( _nx, _Ixdot, vec );
    //??_I2vec( _nx, _Ixdot, _np, _Iyq, _vec_adj+_pos_adj );
    break;
  ...
  default:
    ...
    *_MVYZt = -t; // current time
    _ep2x( _nx, _np, _MVYZr, _pref, _MVYZp, _B, _zref, _MVYZz );
    _ep2x( _nx, _nx, _np, _MVYZd, 0, _MVYZr, _By, _pref, _MVYZp, _By+_nx*_nx,
           _yref, _MVYZy );
    _opADJTC  = _pDAG->subgraph( _nx, _pADJCC );
    delete[] _PMIC; _PMIC = new PVT[_opADJTC.size()];
    _CC_I_ELL( _pDAG, _opADJTC, _PMIC, _nx, _pADJCC, _nVAR-_np, _pVAR, _MVYZVAR,
               _MVYZf, _Edy, _Ay, _nz, _yrefdot, _Bydot, _Idydot, _Qydot,
               options.QTOL, machprec() );
    _pDAG->eval( _np, _pADJTC+_nx, _Iyq, _nVAR-_np, _pVAR, _IVAR, true );
    _E2vec( _nx, _nz, _yrefdot, _Qydot, _Bydot, vec );
    //??_E2vec( _nx, _nz, _yrefdot, _Qydot, _Bydot, _np, _Iyq, _vec_adj+_pos_adj );
    break;
  }
...
( const OPT &options, unsigned iADJRHS, unsigned iQUAD, unsigned pos_fct, unsigned ifct )
{
  ...
  delete[] _pADJRHS;
#ifndef MC__ODEBNDS_GSL_USE_BAD
  _pADJRHS = _pDAG->FAD( 1, &pHAM, _nx+_np, _pVAR+_nx );
#else
  _pADJRHS = _pDAG->BAD( 1, &pHAM, _nx+_np, _pVAR+_nx );
#endif
  _opADJRHS = _pDAG->subgraph( _nx, _pADJRHS );
  _pADJQUAD = _pADJRHS + _nx;
  _opADJQUAD = _pDAG->subgraph(_np, _pADJQUAD);
...
( const OPT &options, double t, const REALTYPE* y, REALTYPE* ydot, REALTYPE* vec_sta )
{
  if( !_pADJRHS ) return false; // **error** RHS not defined
  ...
  case OPT::NONE:
    ...
    _QUAD_I( _pDAG, _opADJQUAD, _IADJRHS, _np, _pADJQUAD, _nVAR-_np, _pVAR,
             _IVAR, _Iyqdot );
    _pDAG->eval( _opADJRHS, _IADJRHS, _nx, _pADJRHS, _Iydot, _nVAR-_np, _pVAR,
                 _IVAR );
    _I2vec( _nx, _Iydot, ydot );
    //??_I2vec( _nx, _Iydot, _np, _Iyqdot, ydot );
    break;  
  ...
  case OPT::DINEQ:
    ...
    _QUAD_I( _pDAG, _opADJQUAD, _IADJRHS, _np, _pADJQUAD, _nVAR-_np, _pVAR,
             _IVAR, _Iyqdot );
    _RHS_I_DI( _pDAG, _opADJRHSi, _IADJRHS, _nx, _pADJRHS, _nVAR-_np, _pVAR,
               _IVAR, _Iydot, _yLdot, _yUdot);
    _I2vec( _nx, _yLdot, _yUdot, ydot );
    //??_I2vec( _nx, _yLdot, _yUdot, _np, _Iyqdot, ydot );
    break;  
   
  case OPT::ELLIPS:
  default:
    _vec2E( vec_sta, _nx, _np, _Q, _Er, _Ir, _pref, _Ip, _B, _zref, _Iz );
    for( unsigned jx=0; jx<_nx; jx++ )
      _MVYZr[jx].set( _MVYZenv, _nx+jx, _Ir[jx] );
    for( unsigned jp=0; jp<_np; jp++ )
      _MVYZp[jp].set( _MVYZenv, _nx+_nx+jp, _Ip[jp] );
    ...
    //std::cout << "_Er =\n" << _Er << std::endl; { int dum; std::cin >> dum; }

    // Adjoint bounds
    _vec2E( y, _nx, _nx, _np, _Qy, _Edy, _Idy, 0, _Ir, _By, _pref, _Ip,
    ...
    *_MVYZt = -t; // Current time
    _QUAD_I( _pDAG, _opADJQUAD, _IADJRHS, _np, _pADJQUAD, _nVAR-_np, _pVAR,
             _IVAR, _Iyqdot );
    _RHS_I_ELL( _pDAG, _opADJRHS, _PMADJRHS, _nx, _pADJRHS, _nVAR-_np, 
    ...
    //??_E2vec( _nx, _nz, _yrefdot, _Qydot, _Bydot, _np, _Iyqdot, ydot );
...
( const OPT&options, double t, const REALTYPE*y, REALTYPE*qdot, const bool bndinit )
{
  ...
  case OPT::DINEQ:
    if( !bndinit ) break;
    _vec2I( y, _nx, _Iy );// set current adj bounds
    break;

  case OPT::ELLIPS:
  default:
    *_MVYZt = -t;
    _vec2E( y, _nx, _nx, _np, _Qy, _Edy, _Idy, 0, _Ir, _By, _pref, _Ip, // set current adj bounds
            _By+_nx*_nx, _yref, _Iy );
    for( unsigned jy=0; jy<_nx; jy++ ) { _MVYZd[jy].set( _MVYZenv, jy, _Idy[jy] ); }
    _ep2x( _nx, _nx, _np, _MVYZd, 0, _MVYZr, _By, _pref, _MVYZp, _By+_nx*_nx,
           _yref, _MVYZy );
  ...
  _QUAD_I( _pDAG, _opADJQUAD, _IADJRHS, _np, _pADJQUAD, _nVAR-_np, _pVAR, _IVAR, _Iyqdot );
  _I2vec( _np, _Iyqdot, qdot );
   return true;
}
...
( const OPT &options, const PVT* PMp )
{
  ...
  unsigned kp=_np;
  for( unsigned ip=0; ip<_np && kp==_np; ip++ )
    if( PMp[ip].env() ) kp = ip;
  ...
  for( unsigned ip=0; ip<_np; ip++ ) _pVAR[_nx + _nx +ip] = _pP[ip];
  _pVAR[_nx + _nz] = (_pT? *_pT: 0. );
  _Iy = _Iz = _Ip = _Iq = _Iyq = 0; // not used
  for( unsigned ip=0; ip<_np; ip++ ) _PMVAR[_nx + _nx +ip] = PMp[ip];
  _PMy = _PMVAR;
  _PMz = _PMy + _nx;
  _PMp = _PMz + _nx;
  _PMq = _PMp + _np;
  ...
  delete[] _PMyqdot; _PMyqdot = _np? new PVT[_np]: 0;
  delete[] _Ryqdot;  _Ryqdot  = _np? new double[_np]: 0;
  switch( options.WRAPMIT){
  case OPT::NONE:
    break;
  case OPT::DINEQ:
  ...
  default:
    ...
    _MVYZz = _MVYZy + _nx;
    _MVYZp = _MVYZz + _nx;
    for( unsigned ip=0; ip<_np; ip++ ){ _MVYZp[ip].set( _MVYZenv, ip, _PMp[ip].B() ); }
    _MVYZt = _MVYZp + _np;
    if( !_MVZd )     _MVZd      = new PVT[_nx];
    if( !_MVZy )     _MVZy      = new PVT[_nx];
    if( !_MVZVAR )   _MVZVAR    = new PVT[_nz+1];
...
( const OPT &options, const double t, REALTYPE*vec, unsigned pos_fct, unsigned ifct )
{
  ...
  case OPT::DINEQ:
    ...
    //??_PMI2vec( _PMenv, _nx, _PMy, _np, _PMyq, _vec_adj+_pos_adj );
    break;
  ...
  default:
    ...
    //??_PME2vec( _PMenv, _nx, _PMy, _Qy, _np, _PMyq, _vec_adj+_pos_adj );
    break;
  }
...
{
  if( !_vQUAD.size() || !_nq ) return true;
  _pDAG->eval( _np, _pADJTC+_nx, _PMyq, _nz+1, _pVAR+_nx, _PMVAR+_nx );
  _PMI2vec( _PMenv, _np, _PMyq, vec, true );
  return true;
}
...
( const OPT &options, const double t, REALTYPE*vec, unsigned pos_fct, unsigned ifct )
{
  ...
  case OPT::DINEQ:
    ...
    _pDAG->eval( _opADJTC, _PMADJTC, _nx, _pADJCC, _PMxdot, _nVAR-_np, _pVAR, _PMVAR );
    _pDAG->eval( _np, _pADJTC+_nx, _PMyq, _nVAR-_np, _pVAR, _PMVAR, true );
    _PMI2vec( _PMenv, _nx, _PMxdot, vec );
    //??_PMI2vec( _PMenv, _nx, _PMxdot, _np, _PMyq, _vec_adj+_pos_adj );
    break;
  ...
  default:
    ...
    //??_vec2PME( _vec_adj+_pos_adj, _PMenv, _nx, _PMy, _Qy, _Edy, _Idy, _np, _PMyq ); // current adjoint/quadrature polynomial model
    _PMADJTC = new PVT[_opADJTC.size()];
    ...
    if( !options.ORDMIT ){
      ...
      _RHS_PM_ELL0( _pDAG, _opADJTC, _PMIC, _opADJDTC, _IADJDTC, _nx, _pADJCC,
                    _pADJDTC, _nVAR-_np, _pVAR, _PMVAR, _IVAR, _PMxdot, _Idgdy,
                    _Idy, _Ay, _Idydot );
    }
    ...
    // linear part is taken as the mid-point of this matrix
      ...
      *_MVYZt = -_t; // set current time
      ...
      _RHS_PM_ELL1( _pDAG, _opADJTC, _PMIC, _opADJDTC, _PMADJDTC, _nx, _pADJCC,
                    _pADJDTC, _nVAR-_np, _pVAR, _PMVAR, _MVYZVAR, _PMxdot,
    ...
    // of the same order as the parameter polynomial model is computed
      ...
      _e2x( _nx, _MVYZd, _MVYZy, false );
      *_MVYZt = -_t; // set current time   
      _RHS_PM_ELL2( _pDAG, _opADJTC, _PMIC, _nx, _pADJRHS, _nVAR-_np, _pVAR,
                    _MVYZVAR, _PMenv, _PMxdot, _MVYZf, _np, _Idy, _Ay, _Idydot );
    }
    ...
    //??_PME2vec( _PMenv, _nx, _PMxdot, _Qydot, _np, _PMyq, _vec_adj+_pos_adj ); 
    break;
  }
...
()
{
  ...
  // Add initial state contribution to derivative bounds
  _pDAG->eval( _np, _pADJTC, _PMyq, _nVAR-_np, _pVAR, _PMVAR, true );
...
( const OPT &options, double t, const REALTYPE* y, REALTYPE* ydot, REALTYPE* vec_sta )
{
  ...
  case OPT::NONE:
    ...
    _QUAD_PM( _pDAG, _opADJQUAD, _PMADJRHS, _np, _pADJQUAD, _nVAR-_np, _pVAR,
              _PMVAR, _PMyqdot );
    _pDAG->eval( _opADJRHS, _PMADJRHS, _nx, _pADJRHS, _PMydot, _nVAR-_np, _pVAR,
                 _PMVAR );
    _PMI2vec( _PMenv, _nx, _PMydot, ydot );
    //_PMI2vec( _PMenv, _np, _PMyqdot, ydot+_offset_quad);
    //??_PMI2vec( _PMenv, _nx, _PMydot, _np, _PMyqdot, ydot );
    break;  
  case OPT::DINEQ:
    _vec2PMI( vec_sta, _PMenv, _nx, _PMz );
    _vec2PMI( y, _PMenv, _nx, _PMy );// set current adjoint bounds
    _RHS_PM_DI( _pDAG, _opADJRHSi, _PMADJRHS, _nx, _pADJRHS, _nVAR-_np, _pVAR,
                _PMVAR, _PMydot, _RyLdot, _RyUdot);
    _PMI2vec( _PMenv, _nx, _PMydot, _RyLdot, _RyUdot, ydot );
    //??_PMI2vec( _PMenv, _nx, _PMydot, _RyLdot, _RyUdot, _np, _PMyqdot, ydot );
    break;  
  case OPT::ELLIPS:
  default:
    ...
    // Adjoint derivative bounds
    _QUAD_PM( _pDAG, _opADJQUAD, _PMADJRHS, _np, _pADJQUAD, _nVAR-_np, _pVAR, _PMVAR,
              _PMyqdot );
    ...
    if( !options.ORDMIT ){
      ...
      _RHS_PM_ELL0( _pDAG, _opADJRHS, _PMADJRHS, _opADJJAC, _IADJJAC, _nx, _pADJRHS,
                    _pADJJAC, _nVAR-_np, _pVAR, _PMVAR, _IVAR, _PMydot, _Idgdy,
                    _Idy, _Ay, _Idydot );
    }
    ...
    // linear part is taken as the mid-point of this matrix
      ...
      *_MVYZt = -_t; // set current time
      ...
      _RHS_PM_ELL1( _pDAG, _opADJRHS, _PMADJRHS, _opADJJAC, _PMADJJAC, _nx, _pADJRHS,
                    _pADJJAC, _nVAR-_np, _pVAR, _PMVAR, _MVYZVAR, _PMydot,
                    _MVYZdgdy, _Idy, _Ay, _Idydot );
    }
    ...
    // of the same order as the parameter polynomial model is computed
      ...
      _e2x( _nx, _MVYZd, _MVYZy, false );
      *_MVYZt = -_t; // set current time 
      _RHS_PM_ELL2( _pDAG, _opADJRHS, _PMADJRHS, _nx, _pADJRHS, _nVAR-_np, _pVAR,
                    _MVYZVAR, _PMenv, _PMydot, _MVYZf, _np, _Idy, _Ay, _Idydot );
    }

    _RHS_PM_ELL( _nx, _Qy, _Ay, _Idydot, _Qydot, options.QTOL, machprec() );
    _PME2vec( _PMenv, _nx, _PMydot, _Qydot, ydot);
    //??_PME2vec( _PMenv, _nx, _PMydot, _Qydot, _np, _PMyqdot, ydot );
    break;
  }
...
( const OPT&options, double t, const REALTYPE*y, REALTYPE*qdot, const bool reinit )
{
  ...
  if( !options.PMNOREM ) {_PMI2vec( _PMenv, _np, _PMyqdot, qdot, true);}
  else {_PMI2vec( _PMenv, _np, _PMyqdot, 0, qdot );}
