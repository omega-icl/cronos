...
int main() 
{
  ...
  mc::FFVar X1 = P[1], X2 = P[2], S1 = P[3], S2 = P[4], Z = P[5], D = P[0];
  // Auxiliary Equations
  ...
  mc::FFVar c_array[NF];
  F[5] = Leverrier_det( NF , dFdX , c_array );

  //const I Ip[NP] = { I(0.9,1.), I(0.,0.6), I(0.4,1.1), I(0.,5.), I(0.,25.), I(40.,50.), I(30.,55.) };
  const I Ip[NP] = { I(1e-1,1.5), I(0.,0.6), I(0.,0.8), I(0.,5.), I(0.,80.), I(Zin,Zin+1.) };
...
// Steady-State Equilibrium Set
  ...
  ofstream K_un( "Eqset_Zin=50_S2in=80_Cin=0_30000.out", ios_base::out );
...
// Steady-State Bifurcation Points
  ...
  SSBifurcation.options.SETINV.MAX_NODES = 1000;
  ...
  SSBifurcation.open_nodes();

#if defined(SAVE_RESULTS )
  ofstream K_un( "Bitset_Zin=50_S2in=80_Cin=0_30000.out", ios_base::out );
  SSBifurcation.output_nodes( K_un );
  K_un.close();
  //std::list<I*> L_clus = SSBifurcation.clusters();
  //std::cout << "No clusters: " << L_clus.size() << std::endl;
  //ofstream K_cl( "Bifpt_Zin=50_S2in=80_Cin=0_30000.out", ios_base::out );
