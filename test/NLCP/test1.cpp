...
#define  MC__CSEARCH_SHOW_INCLUSION
...
int main() 
{
  ...
  CP.options.BLKDECUSE   = false;//true;
  ...
  std::ofstream K_bnd( "test1_bnd.out", std::ios_base::out );
  CP.output_nodes( K_bnd ); //, true );
  K_bnd.close();
  std::ofstream K_clus( "test1_clus.out", std::ios_base::out );
