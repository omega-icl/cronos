...
{
  // Model Parameters
  const unsigned nc = 3, ns = 5, pf = 3;	//No. stages, feed location, No. components 
  ...
  */
  for( unsigned j = 0; j < ns-1; ++j  ){
    for( unsigned i = 0; i < nc; ++i ){
      F[ j*nc+i+(ns*nc)+nc ] = L[j]*X[i*ns+j] - B*X[i*ns+(ns-1)] - V*( K[i*ns+j+1]*X[i*ns+j+1] ) ;
      for( unsigned i2 = j+1; i2 < ns; ++i2 )
        F[ j*nc+i+(ns*nc)+nc ] += f[ i*ns+i2 ];
...
*/
  ...
  CP.options.RELMETH     = mc::NLCP_GUROBI<I>::Options::DRL;//CHEB;//HYBRID;//DRL;//CHEB;
  CP.options.CMODPROP    = 2; //2
  CP.options.CMODCUTS    = 2; //2
  CP.options.CMREDORD    = 4; //3
  CP.options.CMREDTHRES  = 1e-3;//1e-3
  CP.options.MAXCPU      = 2e4 ;
  std::cout << CP;
