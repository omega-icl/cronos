...
int main()
{
  ...
  for( unsigned i=0; i<NS; i++ ){
    RHS[i*NX+0] = k0*exp(-Ea/(R*T))*(1.-X1);
    RHS[i*NX+1] = (P[1+i] - T)*UA/(CA0*V*Cp) - (1.-X1)*exp(-Ea/(R*T))*(DHR*k0)/Cp;
  }
  ...
  LV0.set_differential( NS, NX, RHS );
...
#if defined( SAVE_RESULTS )
  std::ofstream apprecSTA("test5b_APPROX_STA.dat", std::ios_base::out );
...
#if defined( SAVE_RESULTS )
  std::ofstream direcISTA( "test5b_DINEQI_STA.dat", std::ios_base::out );
...
#if defined( SAVE_RESULTS )
  std::ofstream direcPMSTA( "test5b_DINEQPM_STA.dat", std::ios_base::out );
  std::ofstream direcPMADJ( "test5b_DINEQPM_ADJ.dat", std::ios_base::out );
