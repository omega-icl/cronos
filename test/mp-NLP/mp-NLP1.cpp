...
int main() 
{
  ...
  mc::FFVar *CEQ = C, *CINEQ = C+2, *X = P+2;
  CEQ[0] = 3*sqr(X[0]) + 4*X[0] - 5;
  CEQ[1] = 2*X[1] - 3;
  CINEQ[0] = 2*X[0] + X[1] - P[0] - 2.5;
  CINEQ[1] = .5*X[0] + X[1] - P[1] - 1.5;
  ...
  CP.set_dep( 2, X, CEQ );
#endif
  CP.add_ctr( mc::BASE_OPT::LE, CINEQ[0] );
  CP.add_ctr( mc::BASE_OPT::LE, CINEQ[1] );

  //CP.options.MIPFILE   = "test0.lp";
  CP.options.DISPLAY     = 2;
  CP.options.MAXITER     = 50;
  ...
  const I IP[NP] = { I(0.,1.), I(0.,1.), I(0.,5.), I(0.,5.) };
  CP.setup();
  CP.solve( IP );

#if defined(SAVE_RESULTS )
  std::ofstream ores;
  ores.open( "mp-NLP1.out", std::ios_base::out );
  CP.output_nodes( ores, 7 );
  ores.close();
  //ores.open( "test0b.out", std::ios_base::out );
