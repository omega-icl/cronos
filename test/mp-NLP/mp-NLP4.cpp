...
{
  mc::FFGraph DAG;
  const unsigned NP = 5, NC = 4;
  mc::FFVar P[NP], C[NC];
  for( unsigned int i=0; i<NP; i++ ) P[i].set( &DAG );

  mc::FFVar *CEQ = C, *CINEQ = C+3, *X = P+2, *Y = P+4;
  CEQ[0] = 3*sqr(X[0]) + 4*X[0] - 5 + 0.5*Y[0];
  CEQ[1] = 2*X[1] - 3 + Y[0];
  CEQ[2] = .5*X[0] + X[1] - P[1] - 1.5;
  ...
  const I IP[NP] = { I(0.,1.), I(0.,1.), I(0.,5.), I(0.,5.), I(0.,5.) };
  CP.setup();
  CP.solve( IP );

#if defined(SAVE_RESULTS )
  std::ofstream ores;
  ores.open( "mp-NLP4.out", std::ios_base::out );
